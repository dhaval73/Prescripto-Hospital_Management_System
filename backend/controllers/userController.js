...
const paymentStripe = async (req, res) => {
    try {
        const { appointmentId} = req.body; // Ensure `paymentMethod` is included
...
const verifyStripe = async (req, res) => {
    try {
        const { success, appointmentId, userId, transactionId } = req.body;

        // Check if appointment exists
        const paymentRecord = await PaymentModel.findOne({ appointmentId, userId });
        if (!paymentRecord) {
        ...
        if (success === "true") {
            ...
            paymentRecord.transactionId = transactionId || paymentRecord.transactionId;
